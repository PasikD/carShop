...
struct CarListView: View {
    // ViewModel, который отвечает за логику управления списком автомобилей
    ...
    @State private var selectedCar: CDCar? = nil
    ...
    func carImage(car: CDCar, size: CGFloat) -> some View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                        ...
                        ForEach(filteredCars) { car in
                            if isLoggedIn {
                                HStack {
                                    ...
                                    NavigationLink(destination: EditCarView(car: Binding.constant(car), viewModel: viewModel)) {
                                        HStack {
                                ...
                                // Кнопка для отображения алерта при попытке покупки
                                Button(action: {
                                    showingAlert = true
                                }) {
                                    HStack {
                ...
                .navigationBarTitle("Buy Cars") // Устанавливает заголовок навигационной панели
                .padding(.leading, -5)
                ...
                .sheet(isPresented: $showingAddCar) {
                    // Отображение экрана добавления автомобиля
                    AddCarView(viewModel: viewModel)
                }
                .alert(isPresented: $showingAlert) {
                    // Отображение алерта для покупки автомобиля
                    Alert(
                        title: Text("Contact Us"),
                        message: Text("Для покупки звоните по номеру\n+7 999 999 99 99"),
